...
\begin{figure}[h]
\centerline{\includegraphics[scale=.2]{datasets.png}}
...
Pro vytvoření modelu byla zvolena třída Sequential z modulu torch.nn. Jedním způsobem, jak vytvořit objekt třídy Sequential, je předáním listu s vrstvami modelu. Třída byla rozšířena a umístěna do modulu classifiers. Metoda fit byla stěžejní přidanou metodou inspirovanou zápisem z knihovny Scikit-Learn, jejíž úkolem je natrénovat model. Mezi hlavní parametry, které metoda přebírá patří trénovací, testovací a validační datové sady. Trénovací set je předáván jako objekt třída Dataloader z modulu torch.utils.data, jelikož lze předat objektu velikost dávky. Testovací a validační sety jsou objekty dědící od objektu Dataset z téže modulu. Metoda dále přebírá objekt typu Optimizer z modulu torch.optim pomocí, kterého se aktualizují parametry modelu a kritérium z modulu torch.nn počítající ztrátu modelu. Je předán taky device umožňující zvolit, na kterých zařízení bude model trénovat.

\begin{figure}[ht]
\centerline{\includegraphics[scale=.25]{classifiers.png}}
\caption{Rozšířená třída Sequential}
\label{fig}
\end{figure}

Při trénování jsou každou epochu zaznamenávány různé statistiky, některé z nich jsou vypisovány do konzole v přehledném výpisu. Díky tomu je možné při trénování vidět, v jaké míře se model učí. Jsou vypisovány informace o přesnosti a ztrátě modelu u jednotlivých datových setů.
...
Metoda fit vrací historii trénovaní obsahující statistiky a matice záměn, které jsou vytvořeny během poslední epochy pro trénovací a testovací sety. Řádky matice záměn odpovídají správným třídám a sloupce odpovídají předpovězeným třídám. Jednotlivé prvky matice odpovídají počtu přiřazených vzorků ke třídě. Pokud by byly všechny vzorky přiřazeny správně, tak by byla všechna čísla větší než nula na hlavní diagonále matice.

\subsection{Trénování}

V modulu train byla vytvořena třída Trainer, která měla za úkol obstarávat vše spojené s trénováním. Třída funguje jako Functor, takže je volána jako funkce. Při zavolání třídy je včetně parametrů pro trénování předána také cesta ke složce, kde se mají uložit výsledky trénování. Před zahájením trénování je spuštěno logovaní, které do souboru ukládá informace vypsané do konzole. Po ukončení trénování jsou vytvořeny, pomocí funkce plot z knihovny matplolib, grafy průběhu trénování pro přesnosti a ztrátu. Dále jsou vytvořeny pomocí funkce heatmap z knihovny seaborn grafy matice záměn. Grafy jsou včetně samotného modelu uloženy do složky s výsledky.

\begin{figure}[htbp]
\centerline{\includegraphics[scale=.215]{train.png}}
