...
V následujících oddíle jsou popsány příznaky MFCC, které byly použity v praktické části práce.
...
kde $ ACC $ je přesnost. $ H $ je počet správně klasifikovaných vzorků a $ N $ je celkový počet vzorků.
...
Následující oddíly jsou věnovány vybraným konceptům strojového učení, které jsou použity v praktické části práce. Je zmíněna kriteriální funkce křížová entropie, která je použita pro zjištění ztráty modelu. Popsána je také funkce softmax, která je použita ke výpočtu pravděpodobností pro jednotlivé třídy z výstupu neuronové sítě.
...
Z grafu pro přesnost pro vzorky (viz obr. \mbox{\ref{fig:training_course-baseline}}) lze vyčíst, že byl velký rozdíl, přibližně \mbox{24 \%}, mezi dosaženou nejvyšší přesností pro trénovací a validační sadu. Ztráta modelu postupně klesala na trénovací sadě, avšak lineárně stoupala sadě validační.

Model dosáhl přesnosti pro vzorky na validační sadě \mbox{58,8 \%} (viz tab. \mbox{\ref{tab:best_results-baseline}}) a přesnosti pro nahrávku \mbox{88,5 \%}. Úspěch přesnosti pro nahrávky dokládá i rozložení předpovědí v matici záměn pro validační sadu (viz obr. \mbox{\ref{fig:conf_matrix-val-baseline}}), kde je většina předpovědí umístěna na hlavní diagonále. Zvyšující se ztráta na validační sadě naznačovala, že se model přeučoval na trénovací sadě. Přeučování naznačoval také konstantní průběh přesnosti pro vzorky na validační sadě.
