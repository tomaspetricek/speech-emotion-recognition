...
\begin{acknowledgement} % Grammer checked
...
\begin{sidewaystable}
\centering
\caption{Přehled volně dostupných datových sad~\cite{DBLP:journals/speech/AkcayO20}}
...
\section{Vybrané datové sady} % Grammer checked
...
Vícevrstvý perceptron (MLP) neboli dopředná neuronová síť je složena z více lineárních vrstev. Mezi lineární vrstvy jsou umístěny vrstvy nelineární. Lineární vrstva se skládá z parametrů vah a biasů a nelineární vrstva je tvořena aktivační funkcí~\cite{DBLP:books/lib/Bishop07}.
...
\section{Hyperparametry} % Grammer checked
...
\subsection{Přesnost pro vzorek} % Grammer checked
...
\subsection{Přesnost pro nahrávku} % Grammer checked
...
\section{Vybrané koncepty} % Grammer checked
...
\begin{figure}[htbp]
\centerline{\includegraphics[scale=.2,keepaspectratio]{htk_command.png}}
...
\chapter{Experimentální vyhodnocení} % Grammer checked
...
\section{Výchozí model} % Grammer checked
...
\section{Finální modely} % Grammer checked
...
\note{Zkratky mít definované jenom jednou}
\note{Přidat PNG finálního modelu}
\note{Přidat vygenerované zadání práce ze stagu. Pomocí online nástrojů. PDF splitter a merger. Musí se dát vyhledávat F5.}
\note{Přidat prázdnou stránku}
